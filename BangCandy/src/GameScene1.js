var BangSprite = cc.PhysicsSprite.extend({
    init: function () {
        cc.log("bang");
        this._super();
...
//        sprite.setColor(new cc.c3b(100,0,0));
    ...
    zhadanTimer:function(second){
        ...
        this.schedule(this.timerDown,1);
           this.scheduleOnce(this.zhadanBang,second);
    }   ,
    timerDown:function(){
         this.timerDownNum = this.timerDownNum-1;
        this.labelTimer.setString(this.timerDownNum);
        if(this.timerDownNum<=0){
            this.unschedule(this.timerDown);
        }
    }   ,
    beginBangTimer:function(second){
        this.scheduleOnce(this.removeBang, second);
//        this.beginTimer(second);
    }   ,
    beginTimer: function (second) {
        this.scheduleOnce(this.removeSelf, second);
    },
    zhadanBang:function(){
          this.makeABangzhaAt(this.body.getPos());
        this.removeSelf();
    }   ,
...
//        var emtter = cc.ParticleSystem.create(s_p_baozha);
        ...
        this.getParent().addChild(emtter);
        var myTexture = cc.TextureCache.getInstance().addImage(s_fire);
        emtter.setTexture(myTexture);

         var bangSprite = new BangSprite();
        bangSprite.initWithFile(s_CloseNormal);
        bangSprite.setOpacity(0);
        bangSprite.initBaozha(pos,this.space);
        this.getParent().addChild(bangSprite);
//        bangSprite.beginTimer(1);
        bangSprite.beginBangTimer(1);
...
BangSprite.create1 = function (fileName, rect) {
    var argnum = arguments.length;
    var sprite = new BangSprite();
    if (argnum === 0) {
        if (sprite.init())
            return sprite;
        return null;
...
//        cc.Director.getInstance().resume();
    ...
    addACandySprite:function(){
        var sprite = new BangSprite();
        sprite.initWithFile(s_CloseNormal);
        ...
        sprite.initCandyWithSpace(cp.v(posX,posY), cp.v(velX,velY), this.space);
        this.addChild(sprite);
//        sprite.beginTimer(5);
        return sprite;
    }   ,
...
//        candySprite.setTag(1111);
//        var baozha = shapes[1];
//         this.addACandySprite();
//        space.removeBody(candy.getBody());
//        space.removeShape(candy);
...
//        layerKongzhi.addChild(labelTimer);
    ...
    addSpriteAtPositionAndVel: function (pos, vel) {
        var sprite = new BangSprite();
        sprite.initWithFile(s_CloseNormal);
        sprite.initCandyWithSpace(pos, vel, this.space);
        this.addChild(sprite);
//        sprite.beginTimer(5);
        return sprite;
    },
    addSpriteAtPositionAndVel1: function (pos, vel) {
        var body = new cp.Body(1, cp.momentForCircle(1, 0, 24, cp.v(0, 0)));
        body.setPos(pos);
        body.setVel(vel);
        this.space.addBody(body);
...
//          var body = this.makeZhadanBody(pos);
        ...
        var sprite = new BangSprite();
        sprite.initWithFile(s_Zhadan,cc.rect(0,0,48,48));
        sprite.initTheZhadan(pos,cp.v(0,0),this.space);
        this.addChild(sprite);
        sprite.zhadanTimer(time);
        if(this.zhadans.length<=0){
